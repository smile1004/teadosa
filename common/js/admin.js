  ...
  async function init() {
    try {
      ...
      if (member.role !== 'admin') {
        elements.accessMessage.textContent = '관리자 권한이 없는 계정입니다.';
        elements.accessMessage.classList.add('error');
        window.setTimeout(function () { window.location.replace('/mypage/'); }, 1600);
        return;
      }
      elements.accessMessage.hidden = true;
      elements.content.hidden = false;
      bindEvents();
      await loadMembers();
    } catch (error) {
      showAccessError(error.message || '관리자 페이지를 불러오지 못했습니다.');
    }
  }
