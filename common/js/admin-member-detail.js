(function (window, document) {
  ...
  function renderServices(summary, applications) {
    ...
    list.innerHTML = applications.length ? applications.map(function (item) {
      const typeLabel = item.type === 'precheck' ? '사전검토 신청' : (item.type === 'development' ? '개발행위허가 신청' : '발전사업허가 신청');
  ...
  function serviceStatusLabel(type, status) {
    ...
    return (type === 'precheck' ? precheck : (type === 'development' ? development : license))[status] || status || '-';
  }
