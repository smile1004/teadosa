(function () {
  ...
  async function loadDevelopmentHistory() {
    if (!developmentHistoryMessage || !developmentHistoryList || !auth.getMyDevelopmentRequests) return;
    developmentHistoryMessage.hidden=false; developmentHistoryMessage.classList.remove('error'); developmentHistoryMessage.textContent='신청내역을 확인하고 있습니다.'; developmentHistoryList.hidden=true;
    try { const outcome=await auth.getMyDevelopmentRequests(); const result=outcome.result||{}; if(!outcome.response.ok||!result.success)throw new Error(result.message||'개발행위허가 신청내역을 불러오지 못했습니다.'); renderDevelopmentHistory(result.requests||[]); }
    catch(error){developmentHistoryMessage.textContent=error.message||'개발행위허가 신청내역을 불러오지 못했습니다.';developmentHistoryMessage.classList.add('error');}
  }
  ...
  function updateProgressOverview() {
    ...
    setText('mypage-active-count', activePrecheck + activeLicense + activeDevelopment + activePpa);
