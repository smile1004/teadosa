(function(){
  ...
  function renderDetailedSections(data,isSample,capacity,area){
    ...
    var rows=document.querySelectorAll('.setback-rows li');var values=[['-','17m','미측정'],['-','666m','이격 대상 아님'],['100m','101m','적합'],['-','784m','미측정'],['-','-','미측정'],['100m','961m','적합']];rows.forEach(function(row,index){var spans=row.querySelectorAll('span,strong,em');if(values[index]&&spans.length>=3){spans[0].textContent=values[index][0];spans[1].textContent=values[index][1];spans[2].textContent=values[index][2];setStatusAppearance(spans[2],values[index][2]);}});setText('roadName','석산1길');setText('smallRoadName','천잠로');setText('setbackSummary','지붕형 태양광은 이격거리 조례 적용 여부를 관할 조례와 설치 형태에 따라 확인합니다.');
  }
  function renderSetbacks(setbacks){
    var rows=document.querySelectorAll('.setback-rows li');var keys=['road','smallRoad','residential','river','forest','heritage'];var measured=0;
    keys.forEach(function(key,index){var item=setbacks&&setbacks[key];var row=rows[index];if(!row)return;var cells=row.querySelectorAll('span,strong,em');if(cells.length<3)return;var hasDistance=item&&item.distance!==null&&item.distance!==undefined&&item.distance!==''&&Number.isFinite(Number(item.distance));cells[0].textContent=item&&item.note||'원천자료 확인 필요';cells[1].textContent=hasDistance?Number(item.distance).toLocaleString('ko-KR')+'m':'-';cells[2].textContent=item&&item.status||'확인 필요';setStatusAppearance(cells[2],cells[2].textContent);if(hasDistance)measured+=1;});
    if(setbacks&&setbacks.road&&setbacks.road.name)setText('roadName',setbacks.road.name);if(setbacks&&setbacks.smallRoad&&setbacks.smallRoad.name)setText('smallRoadName',setbacks.smallRoad.name);
    setText('setbackSummary',measured?(getSiteType()==='roof'?'지붕형 태양광의 이격거리 적용 여부는 관할 조례와 설치 형태에 따라 확인합니다.':measured+'개 항목의 공간 참고거리를 확인했습니다. 조례 기준 판정은 담당자 검수가 필요합니다.'):'V-World 공간정보를 조회하지 못했습니다. 인증키의 2D 데이터 API 권한과 등록 도메인을 확인해 주세요.');
  }
  function setStatusAppearance(element,status){element.classList.toggle('pass',status==='적합');element.classList.toggle('excluded',status.indexOf('대상 아님')>-1);}
