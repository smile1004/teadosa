(function(){
  ...
  function renderResult(data,isSample){
    ...
    loadMap(data.latitude,data.longitude,data.parcelGeometry).then(function(){if(!isSample&&data.pnu&&!hasMeasuredSetback(data.setbacks))loadClientSpatialReview(data);});
  }

  function hasMeasuredSetback(setbacks){return setbacks&&Object.keys(setbacks).some(function(key){var value=setbacks[key]&&setbacks[key].distance;return value!==null&&value!==undefined&&value!==''&&Number.isFinite(Number(value));});}
  async function loadClientSpatialReview(data){
    try{
      if(!mapConfig){var configResponse=await fetch('/api/solar-check/config?v=3',{cache:'no-store'});if(configResponse.ok)mapConfig=await configResponse.json();}
      if(!mapConfig||!mapConfig.vworldApiKey)return;
      var parcelPayload=await vworldJsonp({data:'LP_PA_CBND_BUBUN',attrFilter:'pnu:=:'+data.pnu,size:'5'});var parcelFeatures=getVworldFeatures(parcelPayload);var parcel=parcelFeatures[0];
      if(!parcel||!parcel.geometry){parcelPayload=await vworldJsonp({data:'LP_PA_CBND_BUBUN',geomFilter:'POINT('+data.longitude+' '+data.latitude+')',size:'5'});parcelFeatures=getVworldFeatures(parcelPayload);parcel=parcelFeatures[0];}
      var baseGeometry=parcel&&parcel.geometry?parcel.geometry:{type:'Point',coordinates:[Number(data.longitude),Number(data.latitude)]};
      var radius=1400,latGap=radius/111320,lonGap=radius/(111320*Math.cos(Number(data.latitude)*Math.PI/180));var box='BOX('+(Number(data.longitude)-lonGap)+','+(Number(data.latitude)-latGap)+','+(Number(data.longitude)+lonGap)+','+(Number(data.latitude)+latGap)+')';
      var payloads=await Promise.all([vworldJsonp({data:'LT_L_SPRD',geomFilter:box,size:'1000'}),vworldJsonp({data:'LT_C_UO301',geomFilter:box,size:'500'})]);
      var resolved=Boolean(parcel&&parcel.geometry);var setbacks={road:clientDistanceResult(baseGeometry,getVworldFeatures(payloads[0]),resolved?'필지 경계→도로 중심선 참고거리':'주소 중심점→도로 중심선 임시 참고거리'),residential:{distance:null,status:'확인 필요',note:'주택 용도·밀집 호수 판정 필요'},river:{distance:null,status:'확인 필요',note:'하천구역 경계 데이터 연동 필요'},forest:{distance:null,status:'확인 필요',note:'산림 적용 경계와 조례 확인 필요'},heritage:clientDistanceResult(baseGeometry,getVworldFeatures(payloads[1]),resolved?'필지 경계→문화재보호도 경계':'주소 중심점→문화재보호도 임시 참고거리')};
      renderSetbacks(setbacks);if(resolved)drawParcelBoundary(parcel.geometry);
    }catch(error){console.warn('브라우저 V-World 공간정보 조회 실패',error);}
  }
  function vworldJsonp(options){return new Promise(function(resolve,reject){var callback='vworldCallback_'+Date.now()+'_'+Math.floor(Math.random()*100000);var script=document.createElement('script');var timer=setTimeout(function(){cleanup();reject(new Error('V-World 응답 시간 초과'));},12000);function cleanup(){clearTimeout(timer);delete window[callback];if(script.parentNode)script.parentNode.removeChild(script);}window[callback]=function(payload){cleanup();resolve(payload);};var params=new URLSearchParams({service:'data',request:'GetFeature',version:'2.0',key:mapConfig.vworldApiKey,domain:window.location.origin+'/',format:'json',geometry:'true',attribute:'true',crs:'EPSG:4326',page:'1',size:options.size||'100',callback:callback});params.set('data',options.data);if(options.attrFilter)params.set('attrFilter',options.attrFilter);if(options.geomFilter)params.set('geomFilter',options.geomFilter);script.src='https://api.vworld.kr/req/data?'+params.toString();script.onerror=function(){cleanup();reject(new Error('V-World 연결 실패'));};document.head.appendChild(script);});}
  function getVworldFeatures(payload){var collection=payload&&payload.response&&payload.response.result&&payload.response.result.featureCollection;return collection&&Array.isArray(collection.features)?collection.features:[];}
  function clientDistanceResult(base,features,note){if(!features.length)return{distance:null,status:'주변 데이터 없음',note:note};var minimum=Infinity;features.forEach(function(feature){if(feature&&feature.geometry)minimum=Math.min(minimum,clientGeometryDistance(base,feature.geometry));});return Number.isFinite(minimum)?{distance:Math.round(minimum),status:'거리 확인',note:note}:{distance:null,status:'확인 필요',note:note};}
  function clientGeometryDistance(first,second){var a=clientSegments(first),b=clientSegments(second);if(!a.length||!b.length)return Infinity;var latitude=a[0][0][1]||36,cos=Math.cos(latitude*Math.PI/180),minimum=Infinity;function pointDistance(point,start,end){var p=[point[0]*111320*cos,point[1]*111320],s=[start[0]*111320*cos,start[1]*111320],e=[end[0]*111320*cos,end[1]*111320],dx=e[0]-s[0],dy=e[1]-s[1],t=dx||dy?Math.max(0,Math.min(1,((p[0]-s[0])*dx+(p[1]-s[1])*dy)/(dx*dx+dy*dy))):0;return Math.hypot(p[0]-(s[0]+t*dx),p[1]-(s[1]+t*dy));}a.forEach(function(x){b.forEach(function(y){minimum=Math.min(minimum,pointDistance(x[0],y[0],y[1]),pointDistance(x[1],y[0],y[1]),pointDistance(y[0],x[0],x[1]),pointDistance(y[1],x[0],x[1]));});});return minimum;}
  function clientSegments(geometry){var lines=[];function visit(coordinates){if(!Array.isArray(coordinates))return;if(coordinates.length>=2&&coordinates[0]&&typeof coordinates[0][0]==='number'){for(var i=1;i<coordinates.length;i++)lines.push([coordinates[i-1],coordinates[i]]);return;}coordinates.forEach(visit);}if(geometry.type==='Point'){var p=geometry.coordinates,tiny=.00000001;return[[[p[0]-tiny,p[1]],[p[0]+tiny,p[1]]]];}visit(geometry.coordinates);return lines;}
