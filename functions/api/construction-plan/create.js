...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const capacityType = enumValue(body.capacityType, ['under10mw', 'over10mw', 'unknown']);
    ...
    if (!capacityType) return error('설비용량 구분을 선택해 주세요.');
    ...
    const duplicate = await env.DB.prepare(`SELECT id,request_no FROM construction_plan_requests WHERE member_id=? AND site_address=? AND status NOT IN ('completed','cancelled') LIMIT 1`).bind(auth.member.member_id, siteAddress).first();
    if (duplicate) return jsonResponse({ success: false, code: 'DUPLICATE_CONSTRUCTION_PLAN_REQUEST', message: '같은 사업지 주소로 진행 중인 공사계획신고 신청이 있습니다.', request: { id: duplicate.id, requestNo: duplicate.request_no } }, 409);
    ...
    const safeBody = {
      ...
      capacityType, userType, requestNote: text(body.requestNote, 1500),
      privacyConsent: true, applicationConfirmed: true
    };

    const insert = await env.DB.prepare(
      `INSERT INTO construction_plan_requests(request_no,member_id,applicant_name,phone,email,business_reg_number,site_address,capacity_type,user_type,form_version,form_data,privacy_consented_at,status,submitted_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,'received',?,?)`
    ).bind(temporaryNo, auth.member.member_id, applicantName, phone, email, businessRegNumber || null, siteAddress, capacityType, userType, text(body.formVersion, 50) || 'CONSTRUCTION_PLAN_APPLY_V1', JSON.stringify(safeBody), nowIso, nowIso, nowIso).run();
    ...
    const updated = await env.DB.prepare('UPDATE construction_plan_requests SET request_no=?,updated_at=? WHERE id=? AND member_id=?').bind(requestNo, nowIso, id, auth.member.member_id).run();
    if (!updated.success || Number(updated.meta?.changes || 0) !== 1) throw new Error('신청번호 생성에 실패했습니다.');

    await env.DB.prepare(`INSERT INTO construction_plan_status_history(request_id,from_status,to_status,customer_notice,changed_by,changed_at) VALUES(?,NULL,'received',?,?,?)`)
      .bind(id, '공사계획신고 서비스 신청이 정상적으로 접수되었습니다.', auth.member.member_id, nowIso).run();

    await createAdminNotification(env, {
      type: 'construction_plan_request', title: '새 공사계획신고 신청',
      message: `${applicantName}님의 공사계획신고 신청이 접수되었습니다. (${requestNo})`,
      linkUrl: '/admin/construction-plan/detail/?id=' + encodeURIComponent(id),
      entityType: 'construction_plan', entityId: id, createdAt: nowIso
    });

    return jsonResponse({ success: true, code: 'CONSTRUCTION_PLAN_REQUEST_CREATED', message: '공사계획신고 서비스 신청이 완료되었습니다.', request: { id, requestNo, status: 'received', submittedAt: nowIso } }, 201);
  } catch (err) {
    console.error('공사계획신고 신청 저장 오류:', err);
...
function createRequestNo(d, id) { const k = new Date(d.getTime() + 32400000); return `CPR-${k.getUTCFullYear()}${String(k.getUTCMonth() + 1).padStart(2, '0')}${String(k.getUTCDate()).padStart(2, '0')}-${String(id).padStart(6, '0')}`; }
function error(message, status = 400) { return jsonResponse({ success: false, code: 'INVALID_CONSTRUCTION_PLAN_REQUEST', message }, status); }
