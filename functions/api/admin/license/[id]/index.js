...
export async function onRequestGet({ request, env, params }) {
  try {
    ...
    const history=await env.DB.prepare(`SELECT h.id,h.from_status,h.to_status,h.customer_notice,h.changed_at,m.name changed_by_name FROM generation_license_status_history h LEFT JOIN members m ON m.id=h.changed_by WHERE h.request_id=? ORDER BY h.changed_at DESC,h.id DESC`).bind(id).all();
