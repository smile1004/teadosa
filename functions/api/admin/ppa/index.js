...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const sums = await env.DB.prepare('SELECT status,COUNT(*) count FROM ppa_requests GROUP BY status').all();
    const rows = await env.DB.prepare(`SELECT r.id,r.request_no,r.applicant_name,r.phone,r.email,r.site_address,r.user_type,r.status,r.submitted_at,r.updated_at,m.username,m.member_type FROM ppa_requests r LEFT JOIN members m ON m.id=r.member_id ${where} ORDER BY r.submitted_at DESC,r.id DESC LIMIT ? OFFSET ?`).bind(...bindings, pageSize, offset).all();
    ...
    return jsonResponse({
      success: true, code: 'ADMIN_PPA_REQUESTS_LOADED',
      requests: (rows.results || []).map(x => ({ id: x.id, requestNo: x.request_no, applicantName: x.applicant_name, phone: x.phone, email: x.email, siteAddress: x.site_address, userType: x.user_type, status: x.status, submittedAt: x.submitted_at, updatedAt: x.updated_at, username: x.username, memberType: x.member_type })),
